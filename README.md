...
curl -X POST "http://localhost:8088/task/add" \
  ...
  -d "execute_at=2026-05-01 10:00:00"
```
...
curl -X POST "http://localhost:8088/task/add" \
  ...
  -d "execute_at=$(date -v+30M '+%Y-%m-%d %H:%M:%S')"
```
...
curl -X POST "http://localhost:8088/task/add" \
  ...
  -d "payload={\"order_id\":12345}" \
  -d "execute_at=$(date -v+15M '+%Y-%m-%d %H:%M:%S')"
```
...
curl -X POST "http://localhost:8088/task/add" \
  ...
  -d "execute_at=2026-05-02 00:00:00"
```
...
curl -X POST "http://localhost:8088/task/add" \
  ...
  -d "execute_at=$(date -v-3d '+%Y-%m-%d 09:00:00')"
```
...
// 改为
sched.Executor = executor.NewExecutor(50)
```
